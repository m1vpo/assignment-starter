...
(au lieu d'un zéro) respectez A LA LETTRE les directives indiquées en dessous :
   ...
   Après avoir modifier ce source entre les markeurs indiqués en bas, executer le code depuis la console IPython comme suit :
       ...
       %run AutoContrast_quantiles.py -i image_source.jpg -o image_resultat.jpg 
...
'''
fonction AutoContrast_quantiles()
implémente l’opération de contraste automatique sur une image. 

paramètres :
   inputfile  : nom de fichier de l'image source
   outputfile : nom de fichier de l'image résultat
'''
def AutoContrast_quantiles(inputfile, outputfile):
...
def main(argv):
   ...
   try:
      ...
      print('AutoContrast_quantiles.py -i <inputfile> -o <outputfile>')
      sys.exit(2)
   for opt, arg in opts:
      if opt == '-h':
         print('AutoContrast_quantiles.py -i <inputfile> -o <outputfile>')
   ...
   AutoContrast_quantiles(inputfile, outputfile)
